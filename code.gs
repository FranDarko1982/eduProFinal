...
<div class="profile-grid">
  ...
  <div class="backup-card card">
    <h3>Correo de backup</h3>
    <input id="backupEmail" type="email" placeholder="Correo de backup" />
  </div>
...
const UserApp = {
  loadProfile(){
    ...
    google.script.run
      .withSuccessHandler(data => {
        ...
        qs('#profileCamp').value = (data.campaña ?? data.campana) || '';
        qs('#adminRole').textContent = data.rol || '';
        qs('#adminCamp').textContent = (data.campaña ?? data.campana) || '';
        qs('#lastLogin').textContent = (data.last_login ?? data.lastLogin ?? '') || '';
        qs('#backupEmail').value = data.backup_email || '';
        if(data.prefs){
      ...
      .withFailureHandler(err => console.error('getUserProfile ERROR:', err))
  ...
  saveProfile(){
    const data = {
      ...
      campana: qs('#profileCamp').value,
      backup_email: qs('#backupEmail').value,
      prefs: {
    ...
    google.script.run
      .withSuccessHandler(()=>toast('Perfil actualizado'))
      .withFailureHandler(err => alert('Error al guardar: ' + err.message))
    ...
    google.script.run
      .withSuccessHandler(rows=>{
        ...
        (rows||[]).forEach(r=>{
          ...
          tr.innerHTML=`<td>${r.accion ?? r.action ?? ''}</td><td>${r.fecha ?? r.date ?? ''}</td>`;
          tb.appendChild(tr);
        });
      })
      .withFailureHandler(err => console.error('getUserActivity ERROR:', err))
  ...
  loadMetrics(){
    ...
    google.script.run
      .withSuccessHandler(data=>{
        ...
        if(!data) return;
        cont.innerHTML = `
          <div class="metric-card">30 días: ${data.res30 ?? data.count30 ?? 0}</div>
          <div class="metric-card">90 días: ${data.res90 ?? data.count90 ?? 0}</div>
          <div class="metric-card">Horas reservadas: ${data.horas ?? data.hours ?? 0}</div>
        `;
      })
      .withFailureHandler(err => console.error('getUserMetrics ERROR:', err))
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(['admin','manager'].includes((window.USER_ROLE||'').toLowerCase())){
    qs('#adminBlock').style.display='block';
  }
  // llamadas para cargar datos al abrir la página
  UserApp.loadProfile();
